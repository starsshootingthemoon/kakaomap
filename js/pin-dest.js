/* =========================================================
   pin-dest.js — 도착지 핀 설정 (카카오맵, autoload=false)
   ========================================================= */

const $name   = document.getElementById('dest-name');
const $addr   = document.getElementById('dest-addr');
const $setBtn = document.getElementById('btn-set-dest');
...
/* ── 카카오맵 초기화 ──────────────────────────────────── */
kakao.maps.load(() => {
  const startLat = parseFloat(sessionStorage.getItem('pin_start_lat') || DEFAULT_LAT);
  const startLng = parseFloat(sessionStorage.getItem('pin_start_lng') || DEFAULT_LNG);

  const container = document.getElementById('map');
  const map = new kakao.maps.Map(container, {
    center: new kakao.maps.LatLng(startLat + 0.003, startLng + 0.003),
    level: 3
  });
  ...
  function updateLocation() {
    ...
    getNearbyName(center, name => {
      ...
      /* 주소는 coord2Address로 별도 취득 */
      geocoder.coord2Address(center.getLng(), center.getLat(), (r, s) => {
        const addrText = (s === kakao.maps.services.Status.OK)
          ? ((r[0].road_address && r[0].road_address.address_name) || r[0].address.address_name)
          : '';
        $addr.textContent = addrText;
        sessionStorage.setItem('pin_dest_name', name);
        sessionStorage.setItem('pin_dest_addr', addrText);
        sessionStorage.setItem('pin_dest_lat',  center.getLat());
        sessionStorage.setItem('pin_dest_lng',  center.getLng());
      });
      speak('도착지: ' + name);
      announce('도착지: ' + name);
    });
  ...
  /* 도착지로 설정 → TTS 후 택시 선택으로 이동 */
  $setBtn.addEventListener('click', () => {
    const name = sessionStorage.getItem('pin_dest_name') || '도착지';
    speak(`도착지가 ${name}로 설정되었습니다. 택시를 선택해 주세요.`);
    announce(`도착지: ${name}`);
    setTimeout(() => { window.location.href = 'taxi-type.html'; }, 1200);
  });
});
